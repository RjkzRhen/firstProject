...
#[AllowDynamicProperties] class CSVForm implements PageInterface
{
    ...
    {
        if (!is_array($fields)) {
            return false; // Проверка, что fields является массивом
        }
        foreach ($fields as $field) {
            if (!is_array($field) || !isset($field['isValid']) || !$field['isValid']) {
                return false; // Проверка, что каждый field является массивом и содержит ключ isValid
            }
        }
        return true; // Все поля действительны
    }
    ...
    public function getHtml(): string
    {
        ...
        <meta charset="UTF-8">
        <title>Добавление данных в CSV</title>
        <style type="text/css">
    ...
    <body>
        ...
        $html .= '<input type="submit" name="submit" value="Добавить запись" id="button">';
        $html .= '</form>';;
        return $html;
    }
}